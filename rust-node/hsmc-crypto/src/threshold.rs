...
pub fn reconstruct_secret(shares: &[SecretShare]) -> Result<Scalar, ThresholdError> {
    ...
    for (i, share_i) in shares.iter().enumerate() {
        ...
        let y_i = Scalar::from_canonical_bytes(share_i.value).into_option()
...
impl VssCommitment {
    ...
    pub fn verify_share(&self, share: &SecretShare) -> bool {
        ...
        let y_i = match Scalar::from_canonical_bytes(share.value).into_option() {
            Some(s) => s,
            None => return false,
        };
...
pub fn combine_threshold_schnorr_sigs(
    ...
    for (i, ps) in partial_sigs.iter().enumerate() {
        let s_i = match Scalar::from_canonical_bytes(ps.s_i).into_option() {
            Some(s) => s,
            None => continue,
        };
