...
#   curl -fsSL https://.../seed-bootstrap.sh | sudo bash
# ════════════════════════════════════════════════════════════════════
set -euo pipefail

REPO="${REPO:-https://github.com/XMC-OXR/astranet-network-hub.git}"
DATA_DIR="/var/hsmc"
USER_NAME="hsmc"
...
set -euo pipefail
cd "$DATA_DIR"
[[ -d astranet-network-hub ]] || git clone --depth 1 "$REPO" astranet-network-hub
cd astranet-network-hub/rust-node
...
Type=simple
User=$USER_NAME
WorkingDirectory=$DATA_DIR/astranet-network-hub/rust-node
...
ExecStart=$DATA_DIR/astranet-network-hub/rust-node/target/release/hsmc-node
Restart=always
RestartSec=5
...
echo "   Node ID: $NODE_ID"
echo ""
echo "Next: register this IP in the public seed list at"
echo "      https://hsmc.network/seeds (or PR to seeds.json in the repo)."
