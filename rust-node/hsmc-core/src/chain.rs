...
use crate::{
    ...
    compute_difficulty_adjustment, DIFFICULTY_ADJUSTMENT_WINDOW, TARGET_BLOCK_TIME_SECS,
    MIN_DIFFICULTY, MAX_SUPPLY,
};
...
pub enum ChainError {
    ...
    OrphanBlock    { number: u64, prev_hash: String },
    ChainIntegrityFailure { block_number: u64, reason: String },
    ReorgTooDeep   { depth: usize, max: usize },
    EmptyChain,
    DoubleSpend    { key_image: String, prev_tx: String },
    InsufficientBalance { address: String, required: f64, available: f64 },
    BlockInFuture  { timestamp: i64, max_allowed: i64 },
...
impl std::fmt::Display for ChainError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::OrphanBlock { number, prev_hash } =>
                write!(f, "Orphan block #{}: prev_hash {}... not in chain", number, &prev_hash[..12.min(prev_hash.len())]),
            Self::ChainIntegrityFailure { block_number, reason } =>
                write!(f, "Chain integrity failure at #{}: {}", block_number, reason),
            Self::ReorgTooDeep { depth, max } =>
                write!(f, "Reorg too deep: {} blocks (max {})", depth, max),
            Self::EmptyChain         => write!(f, "Chain is empty"),
            Self::DoubleSpend { key_image, prev_tx } =>
                write!(f, "Double-spend: key image {}... already used in {}", &key_image[..12.min(key_image.len())], &prev_tx[..12.min(prev_tx.len())]),
            Self::InsufficientBalance { address, required, available } =>
                write!(f, "Insufficient balance for {}: required {:.4} have {:.4}", &address[..16.min(address.len())], required, available),
            Self::BlockInFuture { timestamp, max_allowed } =>
                write!(f, "Block timestamp {} exceeds max allowed {}", timestamp, max_allowed),
...
impl Chain {
    ...
    pub fn confirmations(&self, height: u64) -> u64 {
        let tip = self.height();
        if tip >= height { tip - height + 1 } else { 0 }
    }
    ...
    pub fn add_block(&mut self, block: Block) -> Result<(), ChainError> {
        ...
        if self.hash_index.contains_key(&block.hash) {
            return Err(ChainError::DuplicateBlock { number: block.block_number });
        }
        ...
        block.is_valid_full(&tip_hash, tip_height, self.difficulty, median)
    ...
    /// Spends all inputs and creates all outputs.
    fn apply_block_to_utxo_set(&mut self, block: &Block) {
        // Add coinbase UTXO
    ...
    pub fn try_reorg(&mut self, fork_blocks: Vec<Block>) -> Result<usize, ChainError> {
        if fork_blocks.is_empty() { return Ok(0); }
        ...
        let common_ancestor = match self.get_block_by_hash(fork_base_prev) {
            ...
            None    => return Err(ChainError::OrphanBlock {
        ...
        self.total_chain_work = self.blocks
            .iter()
            .map(|b| b.compute_chain_work())
            .sum();

        // Apply new fork blocks
        let fork_len = fork_blocks.len();
        for block in fork_blocks {
            self.add_block(block).map_err(|e| ChainError::InvalidBlock(e.to_string()))?;
        }
    ...
    pub fn register_key_image(
        ...
        self.spent_key_images.insert(key_image.to_string(), tx_hash.to_string());
        Ok(())
    }
    ...
    pub fn avg_block_time(&self, window: usize) -> f64 {
        ...
        if n < 2 { return TARGET_BLOCK_TIME_SECS as f64; }
        ...
        let diffs: Vec<f64> = recent.windows(2)
            ...
            .collect();
        if diffs.is_empty() { return TARGET_BLOCK_TIME_SECS as f64; }
    ...
    pub fn estimated_tps(&self, window: usize) -> f64 {
        ...
        if avg_block_time == 0.0 { return 0.0; }
    ...
    pub fn hashrate_string(&self) -> String {
        ...
        if hr >= 1e12      { format!("{:.2} TH/s", hr / 1e12) }
        else if hr >= 1e9  { format!("{:.2} GH/s", hr / 1e9)  }
        else if hr >= 1e6  { format!("{:.2} MH/s", hr / 1e6)  }
        else if hr >= 1e3  { format!("{:.2} KH/s", hr / 1e3)  }
    ...
    fn update_stats(&mut self) {
        let total_txs: u64 = self.blocks.iter().map(|b| b.transactions_count as u64).sum();
        let total_fees: f64 = self.blocks.iter().map(|b| b.total_fees).sum();
        let supply: f64 = self.blocks.iter().map(|b| b.reward).sum::<f64>().min(MAX_SUPPLY);

        let avg_bt = self.avg_block_time(100);
        let avg_tx = if self.blocks.is_empty() { 0.0 } else {
            total_txs as f64 / self.blocks.len() as f64
        };
        let avg_fee = if total_txs > 0 { total_fees / total_txs as f64 } else { 0.0 };
        let peak_tps = if avg_bt > 0.0 {
            self.blocks.iter().map(|b| b.transactions_count).max().unwrap_or(0) as f64 / avg_bt
        } else { 0.0 };
            ...
            if curr.prev_hash != prev.hash {
                return Err(ChainError::ChainIntegrityFailure {
                    ...
                    reason: format!(
                        ...
                        &prev.hash[..12], &curr.prev_hash[..12.min(curr.prev_hash.len())]
                    ),
                });
            }
            ...
            if computed != curr.hash {
                return Err(ChainError::ChainIntegrityFailure {
                    ...
                    reason: format!(
                        ...
                        &computed[..12], &curr.hash[..12.min(curr.hash.len())]
                    ),
                });
            }
...
mod tests {
    use super::*;
    use crate::{Block, genesis_block, MIN_DIFFICULTY};
    ...
    fn test_utxo_set_balance() {
        ...
        utxo.add(Utxo {
            tx_hash: "txA".into(), output_index: 0, amount: 50.0,
            address: "HSMC_addr_test".into(), block_height: 0, commitment: None,
        });
