/**
 * Deploy WHSMC + BridgeMinter on Polygon Mainnet
 * ==============================================
 *
 * REQUIRED env vars:
 *   DEPLOYER_PRIVATE_KEY       EOA with MATIC for gas
 ...
 *   POLYGONSCAN_API_KEY        For automatic verification
 *
 * Usage:
...
async function main() {
  ...
  console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  console.log(`▶ Network : ${network.name} (Polygon Mainnet)`);
  ...
  const manifest = {
    ...
    nextSteps: [
      ...
      `Verify both contracts on Polygonscan: npx hardhat verify --network polygon ${whsmcAddr} ${admin}`,
      `Verify BridgeMinter: npx hardhat verify --network polygon ${bridgeAddr} ${whsmcAddr} ${admin} [${validators.map(v=>`"${v}"`).join(",")}] ${threshold}`,
