...
export const STORAGE_KEYS = {
  ...
  WALLET_LABEL: '@hsmc/wallet_label',
  CREATED_AT: '@hsmc/created_at',
...
export async function saveWallet(wallet: MobileWallet): Promise<void> {
  ...
  await AsyncStorage.multiSet([
    ...
    [STORAGE_KEYS.STEALTH_ADDRESS, wallet.stealthAddress],
    [STORAGE_KEYS.WALLET_LABEL, wallet.label],
    [STORAGE_KEYS.CREATED_AT, wallet.createdAt],
  ]);
}

export async function loadWallet(): Promise<MobileWallet | null> {
  const keys = [
    STORAGE_KEYS.ENCRYPTED_SEED,
    STORAGE_KEYS.WALLET_ADDRESS,
    STORAGE_KEYS.STEALTH_ADDRESS,
    STORAGE_KEYS.WALLET_LABEL,
  ...
  if (!values[STORAGE_KEYS.ENCRYPTED_SEED] || !values[STORAGE_KEYS.WALLET_ADDRESS]) {
    return null;
  }

  return {
    mnemonic: '', // never stored in plaintext
    encryptedSeed: values[STORAGE_KEYS.ENCRYPTED_SEED]!,
    address: values[STORAGE_KEYS.WALLET_ADDRESS]!,
    stealthAddress: values[STORAGE_KEYS.STEALTH_ADDRESS] || '',
    label: values[STORAGE_KEYS.WALLET_LABEL] || 'My Wallet',
    createdAt: values[STORAGE_KEYS.CREATED_AT] || new Date().toISOString(),
  };
}
...
export async function loadAuthData(): Promise<{ token: string; userId: string } | null> {
  const pairs = await AsyncStorage.multiGet([
    STORAGE_KEYS.AUTH_TOKEN,
    STORAGE_KEYS.USER_ID,
  ]);
  const token = pairs[0]?.[1];
  const userId = pairs[1]?.[1];
