...
export async function streamHsmcAi(config: AdapterConfig): Promise<AdapterResponse> {
  const apiKey = config.apiKey ?? Bun.env.HSMC_AI_KEY;
  if (!apiKey) {
    return { error: "HSMC_AI_KEY environment variable is required", status: 500 };
  }
